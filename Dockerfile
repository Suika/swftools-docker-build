...
RUN apk add --update --no-cache --virtual build-dep build-base git libjpeg-turbo-dev freetype-dev zlib-dev zlib
RUN wget http://swftools.org/swftools-2013-04-09-1007.tar.gz && \
...
RUN LDFLAGS="-static" ./configure && LDFLAGS="-all-static" make
RUN mv src /swftools
